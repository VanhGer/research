...
impl <T: Digest + Default, P: Pairing> Prover<T, P> {
    ...
    ) -> Result<RoundThreeResponse<P>, ProverError> 
    {
        ...
        // Step 6b
        let h_x = b_0_x + f_x.mul(eta) + q_b_x.mul(eta * eta) - DensePolynomial::from_coefficients_slice(&vec![v]);
        let cm1_pi_eta = cq.kzg.commit_g1(&h_x);

        // Step 7
        // [A_0_X]_1 = ∑ cm1_l_i_quotient * a_i
        let mut cm1_a_0 = P::G1::zero();
        for (a_i, index) in a_sparse {
            cm1_a_0.add_assign(cq.cm1_l_i_quotient[*index].mul(a_i));
        }
    ...
    pub fn prove(&mut self, cq: &Cq<P>, t_i: &[P::ScalarField]) -> Result<Proof<P>, ProverError>{
        ...
        for (f_i, value) in self.f_i_map.clone() {
            let index = cq.t_hash_map.get(&f_i).expect("t_hash_map should contain the value of fi");
...
mod tests {
    ...
    fn test_prover_initialization() {
        let f_i = vec![Fr::from(1), Fr::from(2), Fr::from(3), Fr::from(2), Fr::from(3)];
        let prover = Prover::<Sha256, Bls12_381>::new(f_i);

        assert_eq!(prover.f_i.len(), 5);
